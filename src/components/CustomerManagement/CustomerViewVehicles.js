...
function ViewVehicles() {
  ...
  function DeleteVehicle(vehicleNumber) {
    ...
    if (
      window.confirm("Are you sure you want to remove the selected vehicle ?")
    ) {
      const vid = {
        vehicleNumber,
      };
      ...
      console.log(email);
      axios
        .post(`http://localhost:5000/customers/removeVehicle/${email}`, vid)
        .then((res) => {
          getVehicles();
  ...
  function getVehicles() {
    axios
      .get(`http://localhost:5000/customers/getVehicles/${email}`)
      .then((res) => {

...
function UserInfoForm({ user }) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/customers/${email}`)
      .then((res) => {
  ...
  return (
    <div>
      ...
      <br />
      <Button color="danger" onClick={unregToggle}>
        Unregister <IoTrashBin size={19} />
      </Button>
      <Button
      ...
      <Modal isOpen={unregModal} toggle={unregToggle}>
        <ModalHeader toggle={unregToggle}>Unregister</ModalHeader>
        <ModalBody>
